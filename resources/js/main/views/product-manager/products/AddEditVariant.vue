<template>
    ...
    >
        <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'variant_id'">
                <ul>
                    <li
                        v-for="multiVariantId in record.variant_id"
                        :key="multiVariantId"
                    >
    ...
    <a-modal
        ...
        :centered="true"
        :title="
            addEditType == 'add' ? $t('variations.add') : $t('variations.edit')
        "
        @ok="onSubmit"
        :width="modalWidth"
    >
        <a-form layout="vertical">
                    ...
                    <a-row :gutter="16">
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                            <a-form-item name="stock_quantitiy_alert">
                                ...
                                <a-input-number
                                    v-model:value="
                                        addEditForm.stock_quantitiy_alert
                                    "
                                    :placeholder="
            ...
            <a-row :gutter="16" v-if="isUnitPriceEnabled">
                <a-col :xs="24" :sm="24" :md="8" :lg="8">
                    <a-form-item
                        :label="$t('product.units')"
                        name="units"
                        :help="rules.units ? rules.units.message : null"
                        :validateStatus="rules.units ? 'error' : null"
                        class="required"
                    >
                        <a-input-number
                            v-model:value="formData.units_amount"
                            :placeholder="
                                $t('common.placeholder_default_text', [
                                    $t('product.units'),
                                ])
                ...
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                    ...
                    >
                        <a-input-number
                            ...
                            v-model="addEditForm.sales_price"
                            :value="
                                isUnitPriceEnabled
                                    ? formData.sales_price
                                    : addEditForm.sales_price
                            "
                            :placeholder="
                                $t('common.placeholder_default_text', [
            ...
            <a-row :gutter="16">
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    <a-form-item
                        :label="$t('product.barcode_includes')"
                        name="barcode_includes"
                    >
                        <a-row>
                            <a-col :xs="24" :sm="24" :md="24" :lg="24">
                                <a-checkbox
                                    name="isNameIncluded"
                                    v-model="isNameIncluded"
                                    @change="handleCheckboxChange"
                                >
                                    {{ $t("product.name") }}
                                </a-checkbox>
                                <a-checkbox
                                    name="isPriceIncluded"
                                    v-model="isPriceIncluded"
                                    @change="handleCheckboxChange"
                                >
                                    {{ $t("product.price") }}
                                </a-checkbox>
                            ...
                            <a-col :xs="24" :sm="24" :md="12" :lg="12">
                                <a-checkbox
                                    name="isQuantityIncluded"
                                    v-model="isQuantityIncluded"
                                    @change="handleCheckboxChange"
                                >
                                    {{ $t("product.quantity") }}
                                </a-checkbox>
                                <a-input
                                    v-model:value="formData.item_quantity"
                            ...
                            <a-col :xs="24" :sm="24" :md="12" :lg="12">
                                ...
                                <a-input
                                    v-model:value="formData.item_expiry_date"
                        ...
                        >
                            <template #addonAfter>
                                ...
                                <Barcode
                                    ...
                                    :itemExpirationDate="
                                        formData.item_expiry_date
                                    "
                                    :itemCode="addEditForm.item_code"
                                    :itemSalesTotalPrice="totalPrice"
                                    :currencySymbol="currencySymbol"
                                    :isQuantityIncluded="isQuantityIncluded"
                                    :hasExpirationDate="hasExpirationDate"
                                    :isPriceIncluded="isPriceIncluded"
                                    :barcodeSymbology="
...
export default defineComponent({
    ...
    methods: {
        handleCheckboxChange(e) {
            ...
            if (!this.isQuantityIncluded) {
                this.formData.item_quantity = null;
            }
            if (!this.hasExpirationDate) {
                this.formData.item_expiry_date = null;
            }
        },
    },
    setup(props, { emit }) {
        ...
        const isQuantityIncluded = ref(false);
        ...
        const setVariantTableData = (variableTableDataArray) => {
            ...
            forEach(variableTableDataArray, (variableTableDataArr) => {
                ...
                forEach(
                    variableTableDataArr.product_variations,
                    (multiVariant) => {
                        variantIdArray.push(multiVariant.x_variant_id);
                        variantTypeIdArray.push(
                            multiVariant.x_variant_value_id
                        );
                    }
                );
                ...
                newTableData.push({
                    ...
                    purchase_tax_type:
                        variableTableDataArr.details.purchase_tax_type,
                    ...
                    opening_stock_date:
                        variableTableDataArr.details.opening_stock_date,
        ...
        const calculateTotalPrice = () => {
            if (props.isUnitPriceEnabled) {
                const unitPrice = props.category.unit_price;

                totalPrice.value = unitPrice * props.formData.units_amount;
                if (
                    addEditForm.value.sales_tax_type === "exclusive" &&
                    props.formData.tax_id
                ) {
                    const taxRate = props.taxes.filter(
                        (v) => v.xid == props.formData.tax_id
                    )[0].rate;
                    totalPrice.value += (totalPrice.value * taxRate) / 100;
                }
            } else {
                totalPrice.value = addEditForm.value.sales_price;
                if (
                    props.formData.sales_tax_type === "exclusive" &&
                    props.formData.tax_id
                ) {
                    const taxRate = props.taxes.filter(
                        (v) => v.xid == props.formData.tax_id
            ...
            props.formData.sales_price = totalPrice.value;
            addEditForm.value.sales_price = totalPrice.value;
        };
        ...
        const editItem = (record) => {
            addEditForm.value = {
                ...
                variant_id: record.x_variant_id
                    ? record.x_variant_id
                    : record.variant_id,
        ...
        watch(
            () => props.showTable,
            (newVal, oldVal) => {
                isQuantityIncluded.value = props.formData.item_quantity
                    ? true
                    : false;
                hasExpirationDate.value = props.formData.item_expiry_date
                    ? true
                    : false;
        ...
        const variantValueSelected = (value, options) => {
            var selectedParent = find(props.variations, [
                "xid",
                options.parentId,
            ]);
            ...
            forEach(variantValueId.value, (newVariationId) => {
                var isVariantInSelectedVariant = find(
                    selectedParent.sub_variations,
                    {
                        xid: newVariationId,
                    }
                );
