<template>
    ...
    >
        <a-form layout="vertical">
            ...
            <a-row
                :gutter="16"
                v-if="formData.product_type == 'single' && isUnitPriceEnabled"
            >
                <a-col :xs="24" :sm="24" :md="8" :lg="8">
                    <a-form-item
            ...
            <form-item-heading v-if="formData.product_type == 'single'">
                        ...
                        >
                            <template #addonAfter>
                                ...
                                <Barcode
                                    ...
                                    :currencySymbol="appSetting.currency.symbol"
            ...
            <template v-if="formData.product_type == 'variable'">
                ...
                <AddEditVariant
                    :isUnitPriceEnabled="isUnitPriceEnabled"
...
export default defineComponent({
    ...
    methods: {
        selectedCategoryUpdated(value) {
            this.selectedCategory = this.categories.filter(
                (v) => v.xid == value
            )[0];
            this.isUnitPriceEnabled = this.selectedCategory.unit_price
                ? true
                : false;
        ...
        calculateTotalPrice() {
            if (this.isUnitPriceEnabled) {
                this.totalPrice =
                    this.selectedCategory.unit_price *
                    this.formData.units_amount;
                if (
                    this.formData.sales_tax_type === "exclusive" &&
                    this.formData.tax_id
                ) {
                    const taxRate = this.taxes.filter(
                        (v) => v.xid == this.formData.tax_id
                    )[0].rate;
                    this.totalPrice += (this.totalPrice * taxRate) / 100;
                }
            } else {
                this.totalPrice = this.formData.sales_price;
                if (
                    this.formData.sales_tax_type === "exclusive" &&
                    this.formData.tax_id
                ) {
            ...
            this.formData.sales_price = this.totalPrice;
        },
    },
    setup(props, { emit }) {
        ...
        const priceBasedOnUnits = ref(null);
        ...
        const setCategories = (categoryResponseData) => {
            ...
            categoryResponseData.map((responseArray) => {
                listArray.push({
                    ...
                    unit_price: responseArray.unit_price,
