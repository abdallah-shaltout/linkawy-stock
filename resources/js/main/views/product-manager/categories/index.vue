...
export default {
    ...
    setup() {
        ...
        const getCategories = () => {
            axiosAdmin
                .get(
                    `categories?fields=id,xid,name,slug,parent_id,x_parent_id,image,image_url,unit_price&order=parent_id asc&limit=10000`
                )
                .then((response) => {
                    ...
                    listArray.forEach((node) => {
                        ...
                        if (!node.x_parent_id)
                            return allCategoriesArray.push(node);
