        ...
        <a-row :gutter="16">
            ...
            <a-col :span="14">
                <a-space :style="{ marginTop: '5px' }">
                    ...
                    <a-button
                        v-if="
                            permsArray.includes('pos_reset_invoice') ||
                            permsArray.includes('admin')
                        "
                        @click="resetPos"
                    >
...
export default {
    ...
    setup() {
        ...
        const handleKeyPress = (e) => {
            if (e.key === "F8" && selectedProducts.value.length > 0) {
                e.preventDefault();
                document.querySelector(".loading-fast-payment").style.display =
                    "block";
                payNow();
            }
        ...
        const selectSaleProduct = (newProduct) => {
            if (!includes(selectedProductIds.value, newProduct.xid)) {
                selectedProductIds.value.unshift(newProduct.xid);

                selectedProducts.value.unshift({
                    ...newProduct,
        ...
        const payNowSuccess = (invoiceOrder) => {
            ...
            invoiceOrder.user = customers.value.filter(
                (u) => u.xid == invoiceOrder.user.xid
            )[0];
