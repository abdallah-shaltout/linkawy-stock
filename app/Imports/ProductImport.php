        ...
        DB::transaction(function () use ($products) {
            ...
            foreach ($products as $product) {
                ...
                if (
                    ...
                    !array_key_exists('wholesale_price', $product) || !array_key_exists('wholesale_quantity', $product) || !array_key_exists('units_amount', $product)
                ) {
                ...
                if ($category->unit_price && !trim($product['units_amount'])) {
                    throw new ApiException('Units Amount Not Found');
                }
                ...
                // Product Details
                $unitsAmount = trim($product['units_amount']) ? (float) trim($product['units_amount']) : null;
                ...
                $salesPrice = (float) trim($product['sales_price']);

                if ($category->unit_price) {
                    if ($salesTaxType == 'inclusive') {
                        $salesPrice =  $category->unit_price ?
                            $category->unit_price * $unitsAmount + ($category->unit_price * $unitsAmount * $tax->rate / 100) :
                            $salesPrice + ($salesPrice * $tax->rate / 100);
                    } else {
                        $salesPrice =  $category->unit_price ?
                            $category->unit_price * $unitsAmount : $salesPrice;
                    }
                }
                ...
                foreach ($allWarehouses as $allWarehouse) {
                    ...
                    $newProductDetails->units_amount = $unitsAmount;
