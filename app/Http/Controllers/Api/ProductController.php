...
class ProductController extends ApiBaseController
{
    ...
    public function stored(Product $product)
    {
        ...
        $unitsAmount = $request->units_amount ? $request->units_amount : null;
        $taxRate = $request->has('tax_id') && $request->tax_id != '' ? Tax::find($request->tax_id) : 0;
        ...
        if ($productType == 'variable') {
            ...
            foreach ($allVariations as $allVariation) {
                ...
                foreach ($allWarehouses as $allWarehouse) {
                    $salesPrice = $allVariation['sales_price'];
                    $salesTaxType = $allVariation['sales_tax_type'];
                    ...
                    if ($salesTaxType == 'inclusive') {
                        ...
                        $salesPrice =  $category->unit_price ?
                            $category->unit_price * $unitsAmount + ($category->unit_price * $unitsAmount * $taxRate / 100) :
                            $salesPrice + ($salesPrice * $taxRate / 100);
                    }
                    ...
                    $productDetails->units_amount = $request->units_amount;
            ...
            foreach ($allWarehouses as $allWarehouse) {
                ...
                $salesTaxType = $request->sales_tax_type;
    ...
    public function export()
    {
        ...
        $contents = Excel::raw(new ProductsExport, \Maatwebsite\Excel\Excel::XLSX);
